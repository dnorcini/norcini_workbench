...
if(helpMenu){
  const helpSummary = helpMenu.querySelector('summary');
  const helpPopover = helpMenu.querySelector('.help-popover');
  const referenceButton = document.getElementById('newNoteBtn');

  // Match the real toolbar buttons instead of maintaining
  // a separate visual style for <summary>.
  if(helpSummary && referenceButton){
    const applyHelpButtonStyle=()=>{
      const s=getComputedStyle(referenceButton);
      ...
      [
        ...
        helpSummary.style[prop]=s[prop];
      });

      helpSummary.style.boxSizing='border-box';
      helpSummary.style.display='inline-flex';
      helpSummary.style.alignItems='center';
      helpSummary.style.justifyContent='center';
      helpSummary.style.cursor='pointer';
      helpSummary.style.listStyle='none';
      helpSummary.style.userSelect='none';
      helpSummary.style.webkitAppearance='none';
      helpSummary.style.appearance='none';
    };

    applyHelpButtonStyle();
    window.addEventListener('resize',applyHelpButtonStyle);
  }

  // Replace temporary/vague help content with the actual
  // commands we are using in Workbench.
  if(helpPopover){
    helpPopover.innerHTML=`
      ...
      <div class="help-section">
        ...
        <div><code>python3 analysis.py</code><span>Run Python script</span></div>

        <div><code>Rscript analysis.R</code><span>Run R script</span></div>

        <div><code>bash script.sh</code><span>Run shell script</span></div>
        ...
        <div><code>pdflatex paper.tex</code><span>Single LaTeX pass</span></div>
      </div>
  ...
  // Normal dropdown behavior: clicking elsewhere closes it.
...
async function refreshPreview(){
  ...
  if(currentPath && currentPath.endsWith('.C')){
    preview.removeAttribute('src');
