...
if(helpMenu){
  ...
  const terminalHere=document.getElementById('terminalHereBtn');

  // Put Help at the end of the main toolbar.
  if(terminalHere){
    terminalHere.insertAdjacentElement('afterend',helpMenu);
  }
...
async function refreshPreview(){
  ...
  if(isPlotProducingCode){
    try{
      ...
      // ROOT macros always use the output view.
      // Other code files use it when plots actually exist.
      const hasRunRecord=lastRunStartedAt.has(currentPath);

      if(
        (isRootMacro && !hasRunRecord) ||
        (hasRunRecord && outputs.length)
      ){
        preview.removeAttribute('src');
